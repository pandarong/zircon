...
// region allocator.
//
// TODO: Figure out a way to test IRQs in the same manner, without hardcoding
// target-specific IRQ vectors in these tests. That information is stored in
// the kernel and is not exposed to userspace, so we can't simply guess/probe
// valid vectors like we can MMIO and still assume the tests are valid.
...
static bool probe_address_space(void) {
    ...
    for (uint64_t base = 0; base < UINT64_MAX - step; base += step) {
        zx::resource handle;
        status = zx::resource::create(*zx::unowned_resource(root()), ZX_RSRC_KIND_MMIO, base,
                                      mmio_test_size, 0, NULL, 0, &handle);
        if (status == ZX_OK) {
            mmio_test_base = base;
            break;
        }
...
static bool test_basic_actions(void) {
    ...
    EXPECT_EQ(zx::resource::create(*zx::unowned_resource(root()), ZX_RSRC_KIND_ROOT, 0, 0, 0,
    ...
    ASSERT_EQ(zx::resource::create(new_root, kind, mmio_test_base, mmio_test_size, flags,
...
static bool test_invalid_args(void) {
    ...
    EXPECT_EQ(zx::resource::create(*zx::unowned_resource(root()), ZX_RSRC_KIND_MMIO, mmio_test_base,
                                   mmio_test_size, 0, NULL, 0, &temp),
              ZX_OK, "");
    EXPECT_EQ(zx::resource::create(temp, ZX_RSRC_KIND_ROOT, 0, 0, 0, NULL, 0, &fail_hnd),
              ZX_ERR_ACCESS_DENIED, "");
    EXPECT_EQ(zx::resource::create(temp, ZX_RSRC_KIND_MMIO, mmio_test_base, mmio_test_size, 0, NULL,
                                   0, &fail_hnd),
              ZX_ERR_ACCESS_DENIED, "");

    // test invalid kind
    EXPECT_EQ(zx::resource::create(*zx::unowned_resource(root()), ZX_RSRC_KIND_COUNT, mmio_test_base,
                               mmio_test_size, 0, NULL, 0, &temp),
              ZX_ERR_INVALID_ARGS, "");
    EXPECT_EQ(zx::resource::create(*zx::unowned_resource(root()), ZX_RSRC_KIND_COUNT + 1,
                                   mmio_test_base, mmio_test_size, 0, NULL, 0, &temp),
              ZX_ERR_INVALID_ARGS, "");

    // test invalid base
    EXPECT_EQ(zx::resource::create(*zx::unowned_resource(root()), ZX_RSRC_KIND_MMIO, UINT64_MAX, 1024,
                                   0, NULL, 0, &temp),
              ZX_ERR_INVALID_ARGS, "");
    // test invalid size
    EXPECT_EQ(zx::resource::create(*zx::unowned_resource(root()), ZX_RSRC_KIND_MMIO, 1024, UINT64_MAX,
                                   0, NULL, 0, &temp),
              ZX_ERR_INVALID_ARGS, "");
    // test invalid flags
    EXPECT_EQ(zx::resource::create(*zx::unowned_resource(root()), ZX_RSRC_KIND_MMIO, mmio_test_base,
                               mmio_test_size, ZX_RSRC_FLAG_EXCLUSIVE | 0xFF, NULL, 0, &temp),
              ZX_ERR_INVALID_ARGS, "");
...
static bool test_exclusive_shared(void) {
    ...
    EXPECT_EQ(zx::resource::create(*zx::unowned_resource(root()), ZX_RSRC_KIND_MMIO, mmio_test_base,
                                mmio_test_size, ZX_RSRC_FLAG_EXCLUSIVE, NULL, 0, &mmio_1),
              ZX_OK, "");
    EXPECT_EQ(zx::resource::create(*zx::unowned_resource(root()), ZX_RSRC_KIND_MMIO, mmio_test_base,
                               mmio_test_size, 0, NULL, 0, &mmio_2),
              ZX_ERR_NOT_FOUND, "");
    END_TEST;
}
...
static bool test_shared_exclusive(void) {
    ...
    zx::resource mmio_1, mmio_2;
    EXPECT_EQ(zx::resource::create(*zx::unowned_resource(root()), ZX_RSRC_KIND_MMIO, mmio_test_base,
                               mmio_test_size, 0, NULL, 0, &mmio_1),
              ZX_OK, "");
    EXPECT_EQ(zx::resource::create(*zx::unowned_resource(root()), ZX_RSRC_KIND_MMIO, mmio_test_base,
                               mmio_test_size, ZX_RSRC_FLAG_EXCLUSIVE, NULL, 0, &mmio_2),
              ZX_ERR_NOT_FOUND, "");
    END_TEST;
}
...
static bool test_vmo_creation(void) {
    ...
    zx::resource mmio;
    zx::vmo vmo;
    ASSERT_EQ(zx::resource::create(*zx::unowned_resource(root()), ZX_RSRC_KIND_MMIO, mmio_test_base,
                mmio_test_size, 0, NULL, 0, &mmio),
            ZX_OK, "");
...
static bool test_vmo_creation_unaligned(void) {
    ...
    zx::resource mmio;
    zx::vmo vmo;
    ASSERT_EQ(zx::resource::create(*zx::unowned_resource(root()), ZX_RSRC_KIND_MMIO,
                                   mmio_test_base + 3, PAGE_SIZE + 3, 0, NULL, 0, &mmio),
              ZX_OK, "");
    EXPECT_EQ(zx_vmo_create_physical(mmio.get(), mmio_test_base, PAGE_SIZE,
                                     vmo.reset_and_get_address()),
              ZX_OK, "");
...
static bool test_ioports(void) {
    ...
    char io_name[] = "ports!";
    ASSERT_EQ(zx::resource::create(*zx::unowned_resource(root()), ZX_RSRC_KIND_IOPORT, io_base,
                                   io_size, 0, io_name, sizeof(io_name), &io),
              ZX_OK, "");
