...
char devpath[1024] = {};
dc_get_topo_path(dev, devpath, sizeof(devpath));
printf("dc_add_device: %s : %s\n", dev->name, devpath);
...
static zx_status_t dc_get_metadata(device_t* dev, uint32_t type, void* buffer, size_t buflen,
    ...
    // first search dev and its parent devices for a match
    while (dev) {
    ...
    // next check for published metadata with matching path
    char devpath[DC_PATH_MAX];
    zx_status_t r = dc_get_topo_path(dev, devpath, sizeof(devpath));
    if (r < 0) {
        return r;
    }
    list_for_every_entry(&sys_device.metadata, md, dc_metadata_t, node) {
        if (md->path && !strcmp(md->path, devpath) && md->type == type) {
            memcpy(buffer, md->data, md->length);
            *actual = md->length;
            return ZX_OK;
        }
...
static zx_status_t dc_add_metadata(device_t* dev, const char* path, uint32_t type, const void* data,
                                   uint32_t length) {
...
static zx_status_t dc_publish_metadata(device_t* dev, const char* path, uint32_t type,
    ...
    if (dev != &sys_device) {
        // only sys should be publishing metadata
        return ZX_ERR_ACCESS_DENIED;
    }

    return dc_add_metadata(dev, path, type, data, length);
}
...
static zx_status_t dc_handle_device_read(device_t* dev) {
    ...
    case DC_OP_ADD_METADATA: {
        ...
        r = dc_add_metadata(dev, NULL, msg.value, data, msg.datalen);
        break;
    }
