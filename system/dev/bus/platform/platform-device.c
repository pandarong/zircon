...
// we will release their address space resources back to the PhysicalAspaceManager.
...
static zx_status_t pdev_rpc_get_mmio(platform_dev_t* dev,
    ...
    zx_status_t status = zx_resource_create(dev->bus->resource, ZX_RSRC_KIND_MMIO, mmio->base,
                                            mmio->length, 0, dev->name, sizeof(dev->name), &handle);
    if (status != ZX_OK) {
...
static zx_status_t pdev_rpc_get_interrupt(platform_dev_t* dev,
    ...
    zx_handle_t handle;
    pbus_irq_t* irq = &dev->irqs[index];
    zx_status_t status = zx_resource_create(dev->bus->resource, ZX_RSRC_KIND_IRQ, irq->irq, 1,
                                            ZX_RSRC_FLAG_EXCLUSIVE, dev->name, sizeof(dev->name),
                                            &handle);
    if (status != ZX_OK) {
        return status;
    }
...
static zx_status_t platform_dev_rxrpc(void* ctx, zx_handle_t channel) {
    ...
    status = zx_channel_write(channel, 0, &resp, sizeof(resp),
                              (handle_count == 1) ? &handle : NULL, handle_count);
    if (status != ZX_OK) {
