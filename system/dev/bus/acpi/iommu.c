...
static zx_status_t acpi_scope_to_desc(ACPI_DMAR_DEVICE_SCOPE* acpi_scope,
    ...
    switch (acpi_scope->EntryType) {
        ...
        case ACPI_DMAR_SCOPE_TYPE_BRIDGE:
            zxlogf(INFO, "acpi: bridge scopes not supported\n");
    ...
    if (desc_scope->num_hops != 1) {
        zxlogf(INFO, "acpi: non root bus devices not supported\n");
        return ZX_ERR_NOT_SUPPORTED;
    }
...
zx_status_t iommu_manager_init(void) {
    ...
    if (status != ZX_OK) {
        zxlogf(ERROR, "acpi: error %d in zx_iommu_create for dummy\n", status);
        return status;
    }
    ...
    if (acpi_status != AE_OK) {
        zxlogf(INFO, "acpi: could not find DMAR table\n");
    ...
    if (records_start >= records_end) {
        zxlogf(ERROR, "acpi: DMAR wraps around address space\n");
        return ZX_ERR_IO_DATA_INTEGRITY;
    }
    // Shouldn't be too many records
    if (dmar->Header.Length > 4096) {
        zxlogf(ERROR, "acpi: DMAR suspiciously long: %u\n", dmar->Header.Length);
        return ZX_ERR_IO_DATA_INTEGRITY;
    }
    ...
    if (addr != records_end) {
        zxlogf(ERROR, "acpi: DMAR length weird: %u, reached %zu\n", dmar->Header.Length,
        ...
        switch (record_hdr->Type) {
            case ACPI_DMAR_TYPE_HARDWARE_UNIT: {
                ...
                if (whole_segment) {
                    ...
                    status = create_non_whole_segment_iommu_desc(dmar, rec, &desc, &desc_len);
                }
                if (status != ZX_OK) {
                    zxlogf(ERROR, "acpi: Failed to create iommu desc: %d\n", status);
                    goto cleanup;
                }
                ...
                iommu_mgr.iommus[idx].handle = ZX_HANDLE_INVALID;
...
zx_status_t iommu_manager_iommu_for_bdf(uint32_t bdf, zx_handle_t* iommu_h) {
    ...
    // TODO(teisenbe): We currently lazily create the IOMMU handles, since the
    // kernel enables the IOMMU as it constructs the object.  Once we support
    // IOMMUs for all of our peripherals, we should probably do this non-lazily.
    // The big peripheral that we don't support yet is the Intel GPU, which
    // tends to have its own beefier IOMMU.
    if (match->handle == ZX_HANDLE_INVALID) {
        zx_handle_t h;
        status = zx_iommu_create(get_root_resource(), ZX_IOMMU_TYPE_INTEL,
                                 match->desc, match->desc_len, &h);
        if (status != ZX_OK) {
            goto cleanup;
        }
        match->handle = h;
