...
#define IOCTL_SKIP_BLOCK_READ \
    IOCTL(IOCTL_KIND_DEFAULT, IOCTL_FAMILY_SKIP_BLOCK, 2)
#define IOCTL_SKIP_BLOCK_WRITE \
    IOCTL(IOCTL_KIND_DEFAULT, IOCTL_FAMILY_SKIP_BLOCK, 3)
...
// ssize_t ioctl_skip_block_read(int fd, skip_block_rw_operation_t* out);
IOCTL_WRAPPER_OUT(ioctl_skip_block_read, IOCTL_SKIP_BLOCK_READ, skip_block_rw_operation_t);

// ssize_t ioctl_skip_block_write(int fd, const skip_block_rw_operation_t in);
//
// In the event that bad block is grown, the partition will shrink and an
// ZX_ERR_INTERNAL_INTR_RETRY will be returned. The user should retry if
// possible.
IOCTL_WRAPPER_IN(ioctl_skip_block_write, IOCTL_SKIP_BLOCK_WRITE, skip_block_rw_operation_t);
