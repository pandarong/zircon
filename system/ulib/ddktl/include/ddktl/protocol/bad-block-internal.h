...
DECLARE_HAS_MEMBER_FN_WITH_SIGNATURE(has_get_bad_block_list2, GetBadBlockList2,
                                     zx_status_t (C::*)(uint32_t*, uint32_t, uint32_t*));

DECLARE_HAS_MEMBER_FN_WITH_SIGNATURE(has_is_block_bad, IsBlockBad,
                                     zx_status_t (C::*)(uint32_t, bool*));
...
constexpr void CheckBadBlockable() {
    static_assert(internal::has_get_bad_block_list2<D>::value,
                  ...
                  "GetBadBlockList2(uint32_t* bad_blocks, "
                  "uint32_t bad_block_len, uint32_t* num_bad_blocks)");
    static_assert(internal::has_is_block_bad<D>::value,
                  "BadBlockProtocol subclasses must implement "
                  "IsBlockBad(uint32_t block, bool* is_bad)");
